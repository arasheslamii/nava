...
- **Injection (X11):** XTEST typing → clipboard-paste → notify, with automatic per-app paste policy and Unicode-safe routing.
- **ASR:** faster-whisper `small.en` int8 on CPU (~0.45 RTF on a 2015 i7), Silero VAD + anti-hallucination params (silence → empty). `medium.en` available for more accuracy.
...
Local by default, no telemetry, audio never written to disk. Only the latest transcript is cached (for `paste-last`); disable with `[history] keep_last = false`.
...
See [PLAN.md](PLAN.md), [ROADMAP.md](ROADMAP.md), [DECISIONS.md](DECISIONS.md) for the design history.
